...
class CampaignController extends BaseController
{
	...
	{
		$this->view->title = 'Campaign';
		$this->view->page = 'campaign';
		$this->view->mycontroller = 'CampaignController';
					...
					if( isset($data['recent_signatures'][0]) )
					{
						foreach( $data['recent_signatures'] as $c )
						{
							array_push($this->view->recent_signatures, array(
								...
								'period' 	=> $this->time_elapsed_string($c['date'] . ' ' . $c['time']),
							));
						}
					}
	...
	private function time_elapsed_string($datetime, $full = false)
	{
		...
		$string = array(
			'y' => $this->view->dict['YEAR'],
			'm' => $this->view->dict['MONTH'],
			'w' => $this->view->dict['WEEK'],
			'd' => $this->view->dict['DAY'],
			'h' => $this->view->dict['HOUR'],
			'i' => $this->view->dict['MINUTE'],
			's' => $this->view->dict['SECOND'],
		);
		foreach ($string as $k => &$v) {
			if ($diff->$k) {
				$v = $diff->$k . ' ' . $v . (($diff->$k > 1 && $this->view->language == 'en') ? 's' : '');
			} else {
				unset($string[$k]);
			}
		}
		...
		return $string ? implode(', ', $string) . ($this->view->language == 'en' ? ' ' : '') . $this->view->dict['AGO'] : $this->view->dict['JUST_NOW'];
	}
}